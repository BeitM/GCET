...
export default function SimulatorDashboard(){
 ...
 const choose=(id:string)=>{const next=scenarios.find(s=>s.id===id)!;const nextRobot:RobotPresetId=id==="arm"?"arm":id==="intake"?"intake":"launcher";setScenarioId(id as ScenarioId);setGoal(next.goal);setCode(next.code);selectRobot(nextRobot);setIndex(0);setHasRun(false);setAnalysis(null);if(timer.current)clearInterval(timer.current);setRunning(false)};
 const run=()=>{if(timer.current)clearInterval(timer.current);setIndex(0);setAnalysis(null);setRunning(true);let i=0;timer.current=setInterval(()=>{i++;setIndex(i);if(i>=60){if(timer.current)clearInterval(timer.current);setRunning(false);setHasRun(true)}},90)};
 const seek=(nextIndex:number)=>{if(timer.current)clearInterval(timer.current);setRunning(false);setIndex(nextIndex)};
